...
Eight blocking decisions remain: whether a signal carries strength, the shape of a
candidate, strategy versioning, the strategy context, what a dataset is, whether
the leaderboard is stored or recomputed, strategy metadata, and the generator
signature.

Options and trade-offs for each are written out in
`../decisions-to-lock.html`. Each becomes a record here once settled.
